...
geginiData <- function(srvData=NULL, cmpVar=NULL, ineqVar=NULL, groupingVars=NULL, weight=NULL, naomit=TRUE) {
  ...
  for (i in 1:length(cmprs)) {
    ...
    rv<- as.numeric(crosstab(dataValid$ineqVar, dataValid$cmpVar, weights=dataValid$weight, plot=FALSE)$tab[2,])[i] #get the number of  participant (weighted)
    rv.sh<- as.numeric(crosstab(k$categories,k$ineqVar,prop.c=TRUE, weight=k$weight, plot=FALSE)$prop.col[,2])  #get p rate in df!
    strukt<- as.numeric(crosstab(k$categories,k$ineqVar, weight=k$weight, plot=FALSE)$rs)   #get k numbers of df
    tot.sh  <- strukt/sum(strukt)  #get k rate of df
    geginiDatas[[cmprs[i]]]  <- list()
    geginiDatas[[cmprs[i]]][['df']] <- data.frame(p=rv.sh, k=tot.sh)
    rownames(geginiDatas[[cmprs[i]]][['df']])<- attr(crosstab(k$categories,k$ineqVar,prop.c=TRUE, weight=k$weight, plot=FALSE)$rs, "names")
